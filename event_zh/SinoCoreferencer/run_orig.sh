...
cd $CURRENT_PATH
java $EVPROPS -cp entity.jar MentionDetection/MentionDetectionEndToEnd $CURRENT_PATH/$INPUTS
...
cd $CURRENT_PATH
java $EVPROPS -cp entity.jar SVMSemantic/EntitySemanticEndToEnd $CURRENT_PATH/$INPUTS 
...
cd $CURRENT_PATH
java $EVPROPS -cp entity.jar SVMSemantic/ExplainSemanticType $CURRENT_PATH/$INPUTS 1>&-
...
cd $CURRENT_PATH
java $EVPROPS -cp coref.jar ace/rule/RuleCoref $CURRENT_PATH/$INPUTS

echo "Run event trigger identification..."
# run event extraction
cd $CURRENT_PATH
java $EVPROPS -cp event.jar event/trigger/JointTriggerEndToEnd $CURRENT_PATH/$INPUTS
...
java $EVPROPS -cp event.jar event/trigger/ExplainTrigger $CURRENT_PATH/$INPUTS

echo "Run event argument identification..."

java $EVPROPS -cp event.jar event/argument/JointArgumentEndToEnd $CURRENT_PATH/$INPUTS
...
cd $CURRENT_PATH
java $EVPROPS -cp event.jar event/argument/ExplainArg $CURRENT_PATH/$INPUTS

echo "Run event attributes classification..."
# run event attribute classification
java $EVPROPS -cp entity.jar event/attribute/EventAttriEndToEndTest tense $CURRENT_PATH/$INPUTS
java $EVPROPS -cp entity.jar event/attribute/EventAttriEndToEndTest polarity $CURRENT_PATH/$INPUTS
java $EVPROPS -cp entity.jar event/attribute/EventAttriEndToEndTest modality $CURRENT_PATH/$INPUTS
java $EVPROPS -cp entity.jar event/attribute/EventAttriEndToEndTest genericity $CURRENT_PATH/$INPUTS
...
cd $CURRENT_PATH
java $EVPROPS -cp entity.jar event.attribute.ExplainAttribute $CURRENT_PATH/$INPUTS 

cd $CURRENT_PATH
echo "Run event coreference..."
# run event coreference
java $EVPROPS -cp coref.jar ace/event/coref/MaxEntTestEnd2End $CURRENT_PATH/$INPUTS 1>&-
...
cd $CURRENT_PATH
java $EVPROPS -cp coref.jar ace/event/coref/MakeCluster $CURRENT_PATH/$INPUTS 1>&-
